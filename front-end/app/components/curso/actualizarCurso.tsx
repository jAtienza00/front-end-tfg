...
interface actualizarCursoInterface {
  id_curso: number;
  curso: Cursos;
  nombre_curso: string;
...
interface CursoUpdateResponse {
    message: string;
    id: number;
}

export const ActualizarModule: React.FC<actualizarCursoInterface> = ({
  id_curso,
  nombre_curso,
  curso,
  closeActualizar,
  ...
  useEffect(() => {
    ...
    useEffect(() => {
        if (curso) {
            ...
            setColorFondo(curso.color_fondo);
            setColorTexto(curso.color_texto);
            // imagen state remains null unless user selects a new file
        }
    }, [curso]);

  async function actualizarCurso(e: React.FormEvent) {
    e.preventDefault();
        setError(null);
        ...
        if (imagen) { // imagen is the File object from state
        ...
        try {
            const response = await fetch(env.urlPHP() + "/api/cursos/" + id_curso, { 
                method: "POST",
                headers: {
            ...
            const result: CursoUpdateResponse = await response.json();
  ...
  return (
    <div className="container mx-auto p-6 max-w-lg mt-10 mb-10 fixed inset-0 flex justify-center items-center bg-transparent bg-opacity-50">
